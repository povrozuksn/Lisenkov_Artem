...
int main()
    {
        ...
        int Nbut = 6;
        button b[Nbut];
        b[0] = {"Сердце", 1800, 100, false, heart, 1300, 600, 54, 74, 1700, 100, false, 0, 1, 0};
        b[1] = {"Лёгкие", 1800, 200, false, lungs, 1301, 600, 114, 103, 1700, 200, false, 0, 2, 0};
        b[2] = {"Почки", 1800, 300, false, pochki, 1302, 600, 117, 123, 1700, 300, false, 0, 5, 0};
        b[3] = {"Печень", 1800, 400, false, pechen, 1303, 600, 73, 64, 1700, 400, false, 0, 4, 0};
        b[4] = {"Селезёнка", 1800, 500, false, selesenka, 1304, 600, 56, 76, 1700, 500, false, 0, 6, 0};
        b[5] = {"Желудок", 1800, 600, false, stomach, 1305, 600, 76, 87, 1700, 600, false, 0, 3, 0};

        int NButtMenu = 5;
        buttonMenu but[NButtMenu];
        but[0] = {"Перейти к операции", 760, 400, 760, 400};
        but[1] = {"Выйти из программы", 760, 610, 760, 610};
        but[2] = {"Обучение", 760, 540, 760, 540};
        but[3] = {"Настройки", 760, 470, 760, 470};
       //Сделать зашитие тела и баланс денег

        int NButtOp = 1;
        buttonOp BP[NButtOp];
        BP[0] = {"Выйти из программы", 760, 540};

        int NbuttTop = 1;
        buttonTop button[NbuttTop];
        button[0] = {"Заморозить позицию", 30, 50};
            ...
            if(pos == "menu")
             ...
             //игорёк
            ...
            else if(pos == "programm")
              {
                ...
                char str3[100];
                sprintf(str3, "Сердце:%d, Лёгкие:%d, Почки:%d, Печень:%d, Селезёнка:%d, Желудок:%d",
                ...
                char str7[100];
                sprintf(str7, "Пульс: %d", (int)pulse);
                 ...
                 if(txMouseX() > 1700 && txMouseY() > 800 && txMouseX() > 1700 + 100 && txMouseY() < 800 + 50)
                    {
                      ...
                      txSetColor(RGB(255,255,0));
                      txSelectFont("Comic Sans MS", 20);
                      txDrawText(1800, 800, 1800 + 100, 800 + 50, "Меню");
                      if(txMouseButtons() == 1)
                        pos = "menu";
                    }
                if(GetAsyncKeyState(VK_ESCAPE))
                ...
                txSetColor(RGB(255,0,0));
                txSelectFont("Comic Sans MS", 30);
                txDrawText(100, 20, 100 + 400, 20 + 50, "Зашить тело");
                    if(visSue)
                    {
               ...
               else if(pos == "options")
                {
                    ...
                    txTextOut(795, 15, "Настройки");
                    txSetColor(RGB(200,0,0));
                    txSelectFont("Comic Sans MS", 100);
                    txTextOut(790, 10, "Настройки");

                    txSetColor(RGB(100,100,100));
                    txSelectFont("Comic Sans MS", 40);
                    txDrawText(100, 150, 1820, 980, "Здесь так же пусто, как и в моей голове...");
               ...
               else if(pos == "learning")
                {
                    ...
                    txSetColor(RGB(0,0,0));
                    txSelectFont("Comic Sans MS", 100);
                    txTextOut(795, 15, "Обучение");
                    txSetColor(RGB(200,0,0));
                    txSelectFont("Comic Sans MS", 100);
                    txTextOut(790, 10, "Обучение");

                    txSetColor(RGB(100,100,100));
                    txSelectFont("Comic Sans MS", 40);
                    txDrawText(100, 150, 1820, 980, "1) Чтобы орган появился, нажмите на название органа в правой колонке в рабочемм пространстве. \n"
                                                    "2) Чтобы заморозить позицию органа нажмите по нему правой кнопкой мыши. \n"
                                                    "3) Размораживать орган нельзя, а что вы хотели? Так было бы не интересно. \n"
                                                    "4) Чтобы двигать орган наведите на него, затем нажмите, потом двигайте мышью. \n"
                                                    "5) Чтобы получить деньги за зашитие органов, вам необходимо зашить их в определённом порядке. Первый так и быть подскажу, это сердце, а всё остальное находите сами :) \n"
                                                    "6) Слева в программе находятся показатели последовательности в которых вы поставили органы. \n"
                                                    "7) Держите орган при заморозке, это важно! Иначе орган не заморозиться! Держать около 1-2 секунд \n");
                if(GetAsyncKeyState(VK_ESCAPE))
                  {
                    pos = "menu";
                    txSleep(250);
               ...
               else if(pos == "restart")
                {
                    ...
                    txDrawText(710, 370, 1190, 370 + 50, "Ваш резуьтат:");
                    if(pulse < 1)
                      {
                        txSetColor(RGB(0,200,255));
                        txSelectFont("Comic Sans MS", 20);
                        txDrawText(710, 410, 1190, 410 + 20, "Ну, поздрвляю, пульса нет... ты уволен!");
                      }
                    for(i = 0; i < Nbut; i++)
                        {

                        char str4[100];
                        sprintf(str4, "Прибавка: %d $", NumberOfDollarsIfItWasRealMoneyIWouldFuckYourMomAndFatherAndSisterAndBrotherAndStepSisterAndStepBrotherAndStepFatherAndStepMotherAndYourSlaveNiggaAndDogyAndYourCatAndurHampsterAndUrPetFish);
                        txSetColor(TX_GRAY);
                        txSelectFont ("Arial", 20, 0, FW_BOLD);
                    ...
                    txSetColor(RGB(255,0,0));
                    txSelectFont("Comic Sans MS", 30);
                    txDrawText(720, 500, 1180, 500 + 50, "Продолжить");
